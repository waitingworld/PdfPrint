            ...
            for (var pageNumberKey in pageNumberOnly) {
              ...
              if (pageNumber === 0) {
                allPages[pageNumberKey] = await pdfDoc.getPage(pageNumberOnly[pageNumberOnly.length - 1]).then(function (page) {
                  ...
                  printCanvasElt.setAttribute('name', pageNumber)
                  printCanvasElt.width = (0 * PRINT_UNITS);
                  printCanvasElt.height = (0 * PRINT_UNITS);
                  return page.render({
                ...
                allPages[pageNumberKey] = await pdfDoc.getPage(pageNumber).then(function (page) {
                  var viewport = page.getViewport({scale: 1});
                  var printCanvasElt = printContainerElement.appendChild(win.document.createElement('canvas'));
                  printCanvasElt.setAttribute('id', 'print-canvas')
                  printCanvasElt.setAttribute('name', pageNumber)
